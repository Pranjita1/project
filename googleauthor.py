...
class GoogleauthorSpider(scrapy.Spider):
    ...
    def __init__(self):
        self.timepointer = int(time.process_time())
        self.cnt = 0
        self.years = []
        self.start_urls = []
        self.fields = []
        # path of the txt file that has the fields of research
        filepath = 'fields.txt'
        # looping the txt file
    ...
    def parse(self, response):
        # creating a jar of cookies to make the scrapper look like a human
        cookieJar = response.meta.setdefault('cookie_jar', CookieJar())
        cookieJar.extract_cookies(response, response.request)
        # extracting data from the divs
    ...
    def parse_profile_content(self, response):
        ...
        if tmp:
            ...
            yield scrapy.Request(
                "https://scholar.google.com/citations?hl=en&user={user}&cstart={offset}&pagesize=100".format(
                    offset=offset, user=user), callback=self.parse_profile_content,
                meta={'offset': offset, 'user': user, 'total_articles': total_articles, 'years': tyear,
                      'allyears': self.years})
